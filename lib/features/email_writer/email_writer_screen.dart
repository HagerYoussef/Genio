...
class EmailWriter extends StatefulWidget {
  ...
  static String routeName = 'EmailWriter';
...
class _EmailWriterState extends State<EmailWriter> {
  ...
  void didChangeDependencies() {
    ...
    Future.microtask(() async {
      ...
      if (args != null && args is Map && args.containsKey('chatId')) {
        ...
        print("📥 Using passed chatId for EmailWriter: $_chatId");
      } else {
  ...
  Future<bool> checkEmailLimit(BuildContext context) async {
    final prefs = await SharedPreferences.getInstance();
    final isProUser = prefs.getBool('is_pro_user') ?? false;
    final usage = prefs.getInt('email_ai_usage') ?? 0;

    print("📊 Email Usage: $usage | isProUser: $isProUser");
    ...
    await prefs.setInt('email_ai_usage', usage + 1);
    return true;
  }
  ...
  void _sendMessage() async {
    final allowed = await checkEmailLimit(context);
    if (!allowed) return;
    ...
    final reply = await generateEmailWithGemini(prompt);

    setState(() {
      _messages.add({"text": reply ?? "⚠️ Failed to generate email.", "sender": "ai"});
    });
    await _saveChatHistory();
    _scrollToBottom();
  }

  Future<String?> generateEmailWithGemini(String prompt) async {
      ...
      "contents": [
        {
          "role": "user",
          "parts": [
            {"text": "Write a professional email based on the following prompt:\n\n$prompt"}
          ]
        }
      ],
      "generationConfig": {
            ...
            child: Container(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: "Describe the email you want me to write...",
