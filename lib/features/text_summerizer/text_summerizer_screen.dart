...
class TextSummarizer extends StatefulWidget {
  ...
  static String routeName = 'TextSummarizer';
...
class _TextSummarizerState extends State<TextSummarizer> {
  ...
  void didChangeDependencies() {
    ...
    Future.microtask(() async {
      ...
      if (args != null && args is Map && args.containsKey('chatId')) {
        _chatId = args['chatId'];
        await prefs.setString("chat_id_summary", _chatId!);
        print("📥 Using passed chatId for TextSummarizer: $_chatId");
      } else {
        _chatId = prefs.getString("chat_id_summary");

        if (_chatId == null || _chatId!.isEmpty) {
          _chatId = const Uuid().v4();
          await prefs.setString("chat_id_summary", _chatId!);
          print("🆕 Created new summary chatId: $_chatId");
        } else {
          print("📦 Loaded saved summary chatId: $_chatId");
        }
      }
  ...
  Future<void> _saveChatHistory() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('chat_history_$_chatId', jsonEncode(_messages));
  }

  Future<bool> checkSummaryLimit(BuildContext context) async {
    final prefs = await SharedPreferences.getInstance();
    final isProUser = prefs.getBool('is_pro_user') ?? false;
    final usage = prefs.getInt('summary_ai_usage') ?? 0;

    print("📊 Summary Usage: $usage | isProUser: $isProUser");
    ...
    await prefs.setInt('summary_ai_usage', usage + 1);
    return true;
  }

  void _sendMessage() async {
    final allowed = await checkSummaryLimit(context);
    if (!allowed) return;
    ...
    await _saveChatPreviewIfNeeded(input);
    ...
    final summary = await summarizeTextWithGemini(input);
    setState(() {
      _messages.add({"text": summary ?? "⚠️ Failed to summarize.", "sender": "ai"});
    });
    await _saveChatHistory();
    _scrollToBottom();
  }

  Future<String?> summarizeTextWithGemini(String input) async {
      ...
      "contents": [
        {
          ...
          "parts": [
            {
              "text": "Summarize the following content in a concise paragraph:\n\n$input"
            }
          ]
        }
      ],
  ...
  Future<void> _saveChatPreviewIfNeeded(String input) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    if (!allPreviews.containsKey(_chatId)) {
      allPreviews[_chatId!] = {
        "preview": input.split(' ').take(10).join(' '),
            ...
            child: Container(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: "Enter the text to summarize...",
